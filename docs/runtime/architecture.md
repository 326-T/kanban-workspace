...
graph TB
    subgraph vm[組織 VM（シングルテナント）]
        ui[Web UI] --> backend[backend（Kotlin: Ktor + jOOQ）]
        backend --> db[(Postgres<br/>イベントログ)]
        backend --> runner[runner（bun）<br/>エンジン呼び出し特化]
        runner --> c1[Run サンドボックス × N]
        backend --> hooks[Hook サンドボックス（deno）]
    end
...
| コンポーネント | 実装 | 責務 |
|---|---|---|
| backend | Kotlin（Ktor + jOOQ） | **唯一の公開 API**。ドメイン（ユーザ / リソース / ACL / 組織木 / 承認・ボード）・権限コンパイル（D14）・worktree 準備と checkpoint コミット・イベントログの永続化と UI への SSE 再投影・UI（Vite ビルド）の静的配信・スケジューラ |
| runner | bun（TypeScript） | エンジン呼び出しに特化した軽量 API。Agent SDK / codex CLI を駆動して RunEvent を SSE で上流配信。状態は in-flight の Run のみ（永続化しない）。SDK↔CLI の内部プロトコルに触る唯一の場所 |
...
- **runId は backend が採番**し、runner に渡す（イベント相関の主導権は backend）
- runner の API：`POST /runs`（cwd・prompt・コンパイル済み permissions・env を受けて起動）/ `GET /runs/:id/events`（RunEvent の SSE、`id`=seq で Last-Event-ID 再開）/ `POST messages・permissions/:requestId・end`
- **backend が runner の SSE を購読して Postgres に永続化し、UI へは backend 自身の SSE で再投影**する。「ログが真実・配信は投影」の担い手は backend の DB（UI クライアントの再接続・途中参加・サーバ再起動後の再水和はすべてログから復元）
- RunEvent（[runner/src/protocol/events.ts](../../runner/src/protocol/events.ts)）が言語間契約。JSON Schema 化して Kotlin 側の型を生成する

## 言語と役割分担（D16）

- **Kotlin（backend）**：ドメインモデリング（組織木・ACL・承認）と永続化。Ktor + jOOQ
- **bun / TypeScript（runner + Web UI）**：Agent SDK が TS のため、エンジン駆動は特化サービスとして bun に残す。UI は React/TS（Vite ビルドを backend が配信）
...
- **runner イメージは自前ビルド**：Claude Code に公式配布イメージは無い（公式は devcontainer リファレンス + Dev Container Feature）。codex は `ghcr.io/openai/codex-universal` が参考になる。イメージ内 CLI と SDK のバージョン整合、エンジン認証情報（CLI の OAuth）の受け渡しが要設計
